...
rviz_video.py
=============
RViz Marker publisher used with detect_video.py.

detect_video.py publishes UDP packets that already contain world
coordinates (wx, wy, 0.0), so this node uses them directly.

When UDP stops after the video ends, the last batch of ball points is
kept for path-planning inspection.
...
UDP_IP          = "127.0.0.1"
UDP_PORT        = 5005
MARKER_HOLD_SEC = None   # None = keep the last batch after the video ends

# Court parameters matching the default values from demo_final_video_compact.py.
SCENE_D   = 7.0    # court forward depth (m)
SCENE_W   = 6.0    # court left-right width (m)
GRID_STEP = 0.5    # grid spacing (m)

# Net at X=5.0m, width 10ft=3.048m.
NET_X  = 5.0
NET_HW = (10.0 * 0.3048) / 2.0   # half width, 1.524m
...
class MarkerFromUDPVideo(Node):
    def __init__(self):
        super().__init__("marker_from_udp_video")
        ...
        self.get_logger().info(
            f"MarkerFromUDPVideo started  scene={SCENE_D}×{SCENE_W}m  NET@{NET_X}m"
        )
    ...
    def _build_scene_markers(self):
        ...
        net_label.text = f"NET {NET_X:.1f}m"
    ...
    def timer_callback(self):
        ...
        for p in self.latest_points:
            ...
            self.cached_points[track_id] = {"point": p, "seen": now_sec}
        ...
        for track_id, entry in list(self.cached_points.items()):
            ...
            p  = entry["point"]
            wx = float(p["x"])   # already in world coordinates
            wy = float(p["y"])
            wz = 0.03            # place the ball 3cm above the ground
            conf     = p.get("conf", 0.0)
            ...
            sphere.scale.x = sphere.scale.y = sphere.scale.z = 0.065
            sphere.color.r = 1.0
            sphere.color.g = 0.5
            sphere.color.b = 0.0
            sphere.color.a = 0.9
