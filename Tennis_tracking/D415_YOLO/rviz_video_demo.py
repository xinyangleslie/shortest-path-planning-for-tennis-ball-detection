...
UDP_IP   = "127.0.0.1"
UDP_PORT = 5005
MARKER_HOLD_SEC = None   # None = keep the last batch after UDP stops

# =========================
# Camera mounting parameters
# =========================
CAMERA_HEIGHT = 1.676   # 66 inch = 1.676m, matching the demo script
CAMERA_TILT   = 30.0    # measured about 30 degrees; 45 degrees underestimates distance
...
# Court parameters
# =========================
SCENE_W    = 6.0    # court width (m), along the Y axis
SCENE_D    = 7.0    # court depth (m), along the X axis
GRID_STEP  = 0.5    # grid spacing (m)


def cam_to_world(xc, yc, zc):
    """
    Convert camera coordinates to ROS world coordinates and project Z to
    the ground plane (Z=0).
    Intersect the pixel ray direction with the ground plane to reduce the
    effect of depth noise on height.
    dx = xc/zc = (u-cx)/fx, dy = yc/zc = (v-cy)/fy.
    ...
    if denom <= 1e-6:              # ray is parallel to or points away from the ground
        return 0.0, 0.0, 0.0
    t = CAMERA_HEIGHT / denom      # scale to the ray-ground intersection
    X = t * (_cos_t - dy * _sin_t)
    Y = -t * dx
    return X, Y, 0.0               # force Z to ground level
...
class MarkerFromUDP(Node):
    ...
    def _build_scene_markers(self):
        ...
        hw = SCENE_W / 2.0  # the court is centered at Y=0 and X starts at 0
        ...
        # X-direction grid lines, parallel to the Y axis
        x = GRID_STEP
        while x < SCENE_D:
            for y0, y1 in [(-hw, hw)]:
        ...
        # Y-direction grid lines, parallel to the X axis
        y = -hw + GRID_STEP
        while y < hw:
        ...
        # Net at X=5.0m, width 10ft=3.048m
        NET_X     = 5.0
        NET_HW    = (10.0 * 0.3048) / 2.0   # half width, 1.524 m
        net = Marker()
    ...
    def timer_callback(self):
        ...
        # Sync cache from new UDP packets, then freeze the last state after UDP stops.
        if got_packet:
            for old_id in [tid for tid in self.cached_points if tid not in current_packet_ids]:
                self.cached_points.pop(old_id, None)

        marker_array = MarkerArray()
        current_ids  = set()

        # Send DELETEALL on the first frame to clear old markers from previous runs.
        ...
        # Add static court markers first.
        for m in self.scene_markers:
            m.header.stamp = self.get_clock().now().to_msg()
            marker_array.markers.append(m)

        # Add dynamic ball markers.
        expired_ids = []
        ...
        # Delete disappeared trajectory markers.
