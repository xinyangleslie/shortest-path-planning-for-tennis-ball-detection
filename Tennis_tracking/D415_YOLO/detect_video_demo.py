        ...
        while True:
            ...
            for det in raw_dets:
                ...
                cv2.putText(fusion_img, f"Y:{conf:.2f} CV:{cv_score:.2f}",
                            (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1, cv2.LINE_AA)
            ...
            print(
                ...
                f"YOLO:{num_boxes}  dedup:{len(raw_dets)}  active:{active}  color={runtime['input_color']}  rate={playback_rate:.2f}x"
            )
            print(
            ...
            print(f"{'ID':<5}{'u':<7}{'v':<7}{'conf':<7}{'CV':<7}{'x(m)':<8}{'y(m)'}")
            print("-" * 62)
            for tid, tr in sorted(tracks.items()):
                ...
                wx, wy = cam_to_world(tr["pos"][0], tr["pos"][1], tr["pos"][2],
                                      args.camera_height, args.camera_tilt)
                u_, v_ = tr["pixel"]
                print(f"{tid:<5}{u_:<7.0f}{v_:<7.0f}{tr['conf']:<7.2f}{tr['cv_score']:<7.2f}{wx:<8.2f}{wy:.2f}")
