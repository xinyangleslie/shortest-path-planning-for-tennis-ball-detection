...
UDP_IP   = "127.0.0.1"
UDP_PORT = 5005
MARKER_HOLD_SEC = None   # None = UDP details

# =========================
# details
# =========================
CAMERA_HEIGHT = 1.676   # 66 inch = 1.676m details demo details
CAMERA_TILT   = 30.0    # details 30 45 details
...
# details 3m 3m
# =========================
SCENE_W    = 6.0    # details m details Y details
SCENE_D    = 7.0    # details m details X details
GRID_STEP  = 0.5    # details m


def cam_to_world(xc, yc, zc):
    """
    details ROS details Z details Z=0
    details
    dx = xc/zc = (u-cx)/fx dy = yc/zc = (v-cy)/fy details
    details t = H / (sin_t + dy*cos_t)
    """
    if zc <= 0:
        return 0.0, 0.0, 0.0
    dx = xc / zc   # details
    ...
    if denom <= 1e-6:              # details
        return 0.0, 0.0, 0.0
    t = CAMERA_HEIGHT / denom      # details
    X = t * (_cos_t - dy * _sin_t)
    Y = -t * dx
    return X, Y, 0.0               # Z details
...
class MarkerFromUDP(Node):
    def __init__(self):
        ...
        self.prev_ids       = set()
        self.cached_points  = {}
        self.first_callback = True   # details DELETEALL details marker
    ...
    def _build_scene_markers(self):
        ...
        hw = SCENE_W / 2.0  # details Y=0 details X details 0 details
        ...
        # details X=5.0m details10ft=3.048m
        NET_X     = 5.0
        NET_HW    = (10.0 * 0.3048) / 2.0   # details 1.524 m
        net = Marker()
        ...
        for axis_id, dx, dy, r, g, b in [
            (10, 0.3, 0.0, 1.0, 0.0, 0.0),   # X details details
            (11, 0.0, 0.3, 0.0, 1.0, 0.0),   # Y details details
        ]:
            ax = Marker()
    ...
    def timer_callback(self):
        ...
        # details UDP details cache details UDP details
        if got_packet:
        ...
        # details DELETEALL details marker
